...
The build system SHALL produce a runnable Windows `.exe` installer in `.build/windows/` when the build command is executed.

#### Scenario: Windows build succeeds
- **WHEN** a developer runs the local build command on a machine with Node.js 18+
- **THEN** an `.exe` installer file is created in `.build/windows/` that can be executed to install the MDReader application
...
The build system SHALL report a clear error message when required prerequisites are not available.
