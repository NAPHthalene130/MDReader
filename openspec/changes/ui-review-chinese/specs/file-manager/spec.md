...
- **THEN** the file manager displays an empty state with a message in Chinese prompting the user to add a Markdown file
...
- **THEN** the file manager displays the list of recently-read files sorted by last-opened timestamp (most recent first), rendered as styled cards
...
- **WHEN** the user clicks the add button (labeled in Chinese) and selects a `.md` file from the file picker
- **THEN** the file is added to the recently-read list and displayed in the file manager as a styled card with its filename and file path
